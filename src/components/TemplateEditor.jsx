...
const TemplateEditor = ({
  ...
  const editorBox = useMemo(() => {
    const maxW = 760;
    const maxH = 240;
    const scale = Math.min(maxW / (boxWidthCm * CM_TO_PX), maxH / (boxHeightCm * CM_TO_PX));
    return {
      scale,
      widthPx: Math.max(1, Math.round(boxWidthCm * CM_TO_PX * scale)),
      heightPx: Math.max(1, Math.round(boxHeightCm * CM_TO_PX * scale)),
    };
  }, [boxWidthCm, boxHeightCm]);
